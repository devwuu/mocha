...
**✅ 1차 개발을 마치고, 매일 마시는 커피로 직접 써보며 다듬는 중입니다.** 메신저로 커피 한마디 → 조사 → 확인 → 카드 이미지로 배달까지, 핵심 흐름이 처음부터 끝까지 동작해요. 쓰면서 관측된 불편은 그때그때 반영하고 있습니다 — 지난 기록 검색·수정 같은 기능이 그렇게 추가됐어요.
...
2. **모카가 조사하기** — 같이 온 사진(원두 봉투)에서 먼저 읽고, 모자란 건 커피 이름을 단서로 원산지, 가공방식, 로스터리가 소개하는 공식 향미 노트("자몽, 베르가못, 꿀" 같은 것)를 찾아서 채워 넣습니다.
...
기록한 다음에도 말로 다 됩니다. *"지난번에 마신 예가체프 어땠더라?"* 하고 물으면 지난 기록을 찾아서 보여주고, *"그 노트 산미를 낮음으로 고쳐줘"* 하면 저장된 기록도 고쳐줍니다.
...
만드는 도중에도 실사용에서 관측된 불편을 바로바로 반영하며 여러 번 방향을 손봤어요. 처음엔 A4로 인쇄하는 노트였다가 "폰에서 바로 저장·공유"가 진짜 필요란 걸 깨닫고 4:5 카드 배달로 바꿨고, 카드에 넣던 사진은 막상 SNS에 올릴 땐 폰에서 직접 찍은 원본을 쓴다는 걸 확인하고 "정보 읽기 + 날짜별 보관" 역할로 정리했습니다. 쓰다 보니 "지난번 그 커피 어땠더라?"가 궁금해져서 기록 검색과 저장 후 수정도 추가됐어요. 언젠가는 다른 사람들도 쉽게 설치해서 쓸 수 있는 버전을 만들어보고 싶지만, 일단은 제 커피 기록부터 착실히 쌓는 게 목표입니다.
...
**스택**: Java 21 · Spring Boot 4 · Slack SDK(Socket Mode) · OpenAI API(구조화 추출 + 웹 검색 + 사진 OCR, 인터페이스로 추상화) · Thymeleaf(카드 HTML 조립) + Playwright/Chromium(헤드리스로 4:5 카드 JPG 래스터화) · DB 없이 로컬 JSON 파일이 source of truth.
