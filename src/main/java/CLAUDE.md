...
- **Controller / Service / TxService / Repository.** 헥사고날·포트-어댑터·클린 아키텍처 같은 복잡한 구조는 도입하지 않는다. 이 프로젝트 규모(1인용 단일 프로세스)에서는 관리 비용이 이득을 넘는다. 아래 4층은 그 부류가 **아니다** — 패키지를 뒤집거나 포트/어댑터 명명을 만드는 것이 아니라, **이미 존재하던 트랜잭션 경계에 이름을 붙인 것**이다(changes/0029 ADR-77).
  - controller = 파싱·위임·응답 변환만. 얇게. **전송 계층은 둘이다**: REST 컨트롤러(`web/` — changes/0029 TΔ6a부터)와 Slack 수신(`SlackGateway` → `AgentConversationRouter`). Slack 쪽은 TΔ16에서 걷힌다.
    - **둘이 같은 유스케이스를 부른다**: 에이전트 턴은 `agent/turn/TurnRunner`, 노트 저장·조회는 `service/NoteService`다. 전송 계층에 오케스트레이션을 다시 짜지 않는다 — 입구가 둘이 된 순간 그것이 두 벌이 된다는 뜻이고, 실제로 TΔ6a가 라우터에서 턴을 뽑아낸 이유가 그것이다.
    - 전송 계층이 소유하는 것은 **그 매체라서 있는 것**뿐이다: 수신 형식, 응답 배달 방식, 실패를 무엇으로 표현하는가(Slack은 안내 메시지, REST는 실패 상태 코드). 사용자 대면 문구도 여기 산다.
