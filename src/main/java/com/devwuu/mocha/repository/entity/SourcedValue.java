 ...
 * (ref: changes/0028-rdb-storage/delta.md#ADR-72).
 * <p>도메인 {@code Sourced<T>}는 제네릭이지만 컬럼으로 내려가는 것은 {@code Sourced<String>}뿐이다 —
 * {@code officialNotes}({@code Sourced<List<String>>})는 값이 {@code note_official_notes} 테이블로,
 * source는 {@code notes.official_notes_source} 컬럼으로 갈라진다(Q-8). 그래서 이 임베더블은 비제네릭이다.
