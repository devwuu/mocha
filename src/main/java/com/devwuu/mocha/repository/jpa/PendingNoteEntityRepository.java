...
/**
 * 확인 대기 노트의 행 입출력 (ref: changes/0028-rdb-storage/tasks.md TΔ8, delta.md#ADR-73).
 *
 * <p><b>노트가 쓴 3분할(ADR-72)이 여기엔 적용되지 않는다.</b> 그 분할의 근거는 자식 9종의 조인·조립·정렬이
 * 질의로 내려온다는 것인데, pending은 <b>테이블 하나에 사용자당 한 행</b>이고 중첩은 전부 {@code draft}
 * JSONB 안에 접혀 있다(ADR-73 예외) — 자식 행이 없으니 QueryDSL fragment가 맡을 일이 없고 파생 CRUD가
