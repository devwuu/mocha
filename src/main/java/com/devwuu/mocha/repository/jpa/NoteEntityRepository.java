...
/**
 * 노트 애그리거트의 행 입출력 (ref: changes/0028-rdb-storage/delta.md#ADR-73 저장소 3분할, tasks.md TΔ5a).
 *
 ...
 * {@code note} 행에 한정된다.
 *
 * <p>엔티티당 저장소를 두지 않는 이유는 연관 매핑 부재(TΔ3a) + 애플리케이션 전담 삭제 전파(ADR-75)가 곧
...
public interface NoteEntityRepository extends JpaRepository<NoteEntity, Long>, NoteEntityRepositoryCustom {
    ...
    /**
     * 전건 조회 — {@code id} 오름차순.
     * <p>slug가 폐기되면서(ADR-75) 파일 구현체가 쓰던 파일명 정렬이 근거를 잃었다. 대체 키는 삽입 순서를
