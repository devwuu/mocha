 ...
 * 발화는 제안 없이 날짜별 분리 안내 (ref: plan.md#ADR-59, spec FR-15/FR-18/FR-22, data-model.md#V-15).
...
public final class AgentSystemPrompt {
    ...
    public static final String INSTRUCTIONS = """
            ...
            - 기록·수정 대상이 명확하면 추가 확인 질문 없이 즉시 제안(propose)한다 — 미리보기+[저장] 버튼이 곧 확인이다. 대상이 모호하면(후보 복수·엔트리 날짜 복수) 자연어로 짧게 되물어 확정한다. 단, 한 발화에 시음 날짜가 2개 이상 담겨 있으면(예: "2026 07 18"과 "2026 07 19" 헤더가 함께 있는 메모) 즉시 제안의 예외다 — 회차(brews) 규칙의 다중 날짜 안내를 따른다.
            ...
            - 제안 전에 발화에 담긴 시음 날짜가 몇 개인지 먼저 확인한다(메모 형식의 날짜 헤더 포함) — 한 발화에 여러 시음 날짜가 섞이면 제안 tool을 호출하지 않는다. 어느 한 날짜만 골라 기록하지도, 다른 날짜의 시도를 한 날짜의 회차로 합치지도 말고, 한 날짜씩 나눠 보내달라고 안내한다(엔트리 1건 제안 원칙).
