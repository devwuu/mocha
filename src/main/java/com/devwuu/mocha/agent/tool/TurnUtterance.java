 ...
 * <p>{@code segments}는 자동 분해(ADR-61, TΔ3b) 전까지 항상 null — 게이트(TΔ2c)가 시그니처를 다시
 * 흔들지 않도록 자리만 미리 포함한다(findings-TΔ0 §C-4 홀더 제안).
 *
 * @param rawText  이번 턴의 사용자 원문({@code IncomingMessage.text()}) — 사진 캡션은 별도 메시지로
 *                 합류하므로 이 값 하나로 충분하다(§C-5).
 * @param segments 다중 날짜 자동 분해 결과(날짜별 원문 발췌) — 분해 미수행·실패 턴은 null.
 */
