 ...
 * 추출 관찰·진단·다음 계획은 그 회차 {@link Recipe#feedback()}(시도 귀속 맛 코멘트도 review다).
 *
 ...
 */
public record Review(String myTaste, String myTasteOriginal, Rating rating) {
    ...
    /**
     * V-15 정규화: {@code myTaste}가 비어 있으면 review 자체를 {@code null}로 드롭한다
     * (빈 감상 review 금지 — 감상 없는 시도는 review 없이 recipe만 남는다).
     *
     * @return 정규화된 Review, 또는 감상 전무 시 {@code null}.
     */
    public static Review normalize(String myTaste, String myTasteOriginal, Rating rating) {
        if (myTaste == null || myTaste.isBlank()) {
            return null;
        }
        String original = myTasteOriginal == null || myTasteOriginal.isBlank() ? null : myTasteOriginal;
        return new Review(myTaste, original, rating);
    }
}
