 ...
 * <p>보강 규칙(단일 원산지·블렌드 공통으로 로스터리 공식 우선 + 신뢰할 일반 출처 fallback, official_notes
 * 로스터리 출처 한정, FR-3/ADR-14)은 구현체의 검색 지침이 이미 적용한 뒤의 결과다. 블렌드의 여러 구성
 * 원산지는 origin 한 String에 쉼표로 나열돼 온다("에티오피아, 콜롬비아, 브라질", D2).
 *
 * @param roastery      로스터리(검색이 확인/보강한 값, 보통 null — 이미 사용자 언급).
 * @param origin        원산지(단일·블렌드 공통 보강, 블렌드는 여러 원산지를 쉼표 문자열로 나열, D2).
 * @param process       가공 방식.
