 ...
 * <p>썸네일은 <b>파생물</b>이다 — {@code data/photos/<slug>/<date>/*} 원본에서 만들어 {@code site/thumbs/}
 * 아래에 쓰며, 통째로 지워도 리렌더로 재생성된다(ADR-1). 따라서 원본(source of truth)엔 아무것도 남기지 않는다.
 * <p>SiteRenderer(T5-1)가 노트를 렌더할 때 인덱스 카드용으로 호출한다.
 */
public interface ThumbnailGenerator {

    /**
     * 원본 사진에서 썸네일을 생성해 {@code site/thumbs/…}에 쓰고, site 기준 <b>상대 경로</b>를 반환한다.
