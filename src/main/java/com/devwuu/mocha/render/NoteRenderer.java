...
public interface NoteRenderer {
    ...
    /**
     * 수정 세션 날짜 이동 시 옛 날짜의 회차 카드 파생물 전부({@code cards/<접미>/<date>-*.jpg})를
     * 정리한다(ADR-27, AC-39). 파일이 없어도, 노트가 이미 사라졌어도 무해(멱등). 삭제 실패로 남은 카드는
     * {@link #renderAll}({@code --rerender})이 정리하므로 호출부는 실패를 커밋 롤백 사유로 삼지 않는다(plan §7).
     *
     * @param noteId 대상 노트 id.
     * @param date   삭제할 카드의 엔트리 날짜(이동 전 원본 date).
     */
    void removeEntryCard(long noteId, LocalDate date);
}
