...
/**
 * 카드 HTML(시음일 1건) → JPG 래스터화 경계 (ref: plan.md#ADR-11, §3; changes/0002-instagram-share-card TΔ2).
 * <p>NoteRenderer가 시음일 카드를 구울 때 이 경계 뒤로 렌더 엔진(Playwright/Chromium)을 숨긴다 — 카드 HTML은
 * 굽는 순간의 <b>중간 입력</b>이며 파일로 남기지 않는다(ADR-10). 산출물은 {@code cards/<slug>/<date>.jpg}뿐이다.
 * <p>POLICY: 파이프라인·NoteRenderer는 렌더 엔진(Playwright/Chromium) 타입을 직접 참조하지 않는다 —
...
public interface CardImageRenderer {
     ...
     *
     * @param html    카드 HTML 문자열(시음일 1건 렌더 결과). 빈 문자열/null 불가.
     * @param baseDir HTML의 상대 자원이 해석되는 기준 디렉터리(보통 artifact 루트). 없으면 생성한다.
