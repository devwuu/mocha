 ...
 * (ref: plan.md §1 [6]~[7], ADR-3). T3-2 임시 스텁(LoggingConfirmationFlow)을 대체한다.
 * <p><b>이 클래스가 T3-5에서 채우는 것은 [저장]/[취소] 커밋 경로다.</b>
 * <ul>
 ...
 * </ul>
 * <p>{@link #startNewNote}/{@link #revisePending}의 실제 오케스트레이션 배선(추출→매칭→보강→미리보기,
 * 수정→미리보기 갱신)은 아직 미완이다 — 구성요소({@code NoteExtractor}/{@code NoteMatcher}/
 * {@code NoteEnricher}/{@code PendingReviser}/{@code PreviewMessenger})는 존재하지만 이 흐름으로 엮는 일이
 * 남아 있어, T3-5 범위 밖인 두 메서드는 현 동작(로그)을 보존한다.
...
public class DefaultConfirmationFlow implements ConfirmationFlow {
    ...
    public void startNewNote(IncomingMessage message) {
        // TODO(T3-3 배선): NoteExtractor→NoteMatcher→NoteEnricher→PreviewMessenger로 엮는 오케스트레이션 미완.
        log.info("신규 파이프라인 분기(오케스트레이션 미배선): user={} text={}", message.userId(), message.text());
    }
