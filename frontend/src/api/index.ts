 ...
 * 사후 확인이다.
 *
 * S2(갤러리)가 같은 규율을 다시 시작한다 — `getNotes`만 `mock.ts`에서 온다. **TΔ5a가 `GET /api/notes`를
 * 구현하면 아래 import 줄 하나가 `./http`로 옮겨가고 `mock.ts`는 지워진다.** 그때까지도 계약의 정본은
 * mock이 아니라 `src/test/resources/contract/note-list.contract.json`이다.
 */
export { getNoteCandidates, postAgentCancel, postAgentTurn, postNoteCommit, postPhotos } from './http'
export { getNotes } from './mock'
