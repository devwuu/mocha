 ...
 * ① 출처 배지 ② 추출 ml 슬롯 ③ 분쇄도·기구 분리 ④ `feedback` 단일 필드 ⑤ 회차/날짜 추가·엔트리 삭제 부재
 * ⑥ 저장 버튼의 자리와 노트 삭제 구역.
 *
...
export function EditScreen({ noteId, onNavigate }: EditScreenProps) {
   ...
   *
   * 저장 후 폼을 손으로 갱신하지 않고 **응답으로 통째 다시 세우는** 것이 의도다: 날짜 이동은 엔트리를
 ...
 * 회차 삭제·엔트리 삭제는 spec에 없는 동작이다(루트 §3 — `DELETE`는 노트 단위 하나뿐이다).
 */
      ...
      {collapsed ? (
        ...
        <div className="edit__section-body">
          {/*
            ...
            구 V-10은 이동처를 덮어썼는데, 엔트리가 하루치 감상 1건이던 시절(changes/0012)의 규칙이라 회차
          ...
          */}
          ...
          {/* 시안과 갈린 것 ⑥ — 시안은 [저장]을 회차 카드 안에 뒀지만 그 카드가 하나뿐인 그림이다.
              PATCH는 **엔트리 단위**라(회차별 저장 API가 없다) 회차마다 버튼을 두면 각 버튼이 그 날짜
